...
export function PersonCard({ p, contextTag }: { p: Persona; contextTag?: string }) {
  const cumpleHoy = isToday(p.cumpleanos)
  const cumpleParsed = parseCumple(p.cumpleanos)

  if (p.vacante) {
    return (
      <div className="rounded-2xl border border-dashed border-rose-300 bg-rose-50/50 p-4 dark:border-rose-900 dark:bg-rose-500/5">
