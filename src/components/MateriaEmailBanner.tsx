...
export function MateriaEmailBanner({ materia, correos }: { materia: string; correos: string[] }) {
  ...
  return (
    <div className="mb-4 flex flex-wrap items-center gap-2 rounded-2xl border border-amber-100 bg-amber-50/60 px-4 py-3 dark:border-amber-900/40 dark:bg-amber-500/5">
      ...
      <span className="text-sm text-slate-600 dark:text-slate-300">
        Casillas generales de los {correos.length} tribunales de materia{' '}
        <strong className="font-medium text-slate-800 dark:text-slate-100">{materia}</strong>:
      </span>
      <a
        href={buildGroupMailto(correos)}
        title={`Redactar correo a los ${correos.length} tribunales de ${materia} (en copia oculta)`}
