-- Reparación defensiva del login por RUT: vuelve a dejar la tabla
-- "admins" y las funciones de verificación en el estado correcto, sin
-- importar qué parte de las migraciones anteriores alcanzó a correr. Es
-- 100% seguro correr este archivo completo (no duplica ni borra nada que
-- no deba).
...
-- Vuelve a dejar cada RUT con su nombre y clave correctos (corrige
-- cualquier fila que haya quedado a medio guardar).
...
-- Verificación: esta consulta debería devolver 20 filas (rut y nombre de
-- cada administrador). Si devuelve menos, faltan filas por corregir.
select rut, nombre from admins order by nombre;
